...
public class SBOLConverter {
  ...
  public static SBOLDocument convertPartToComponent(Part part, SBOLDocument doc) throws SBOLValidationException {
    ...
    switch (part.partType) {
      case GENE:
        doc.getComponentDefinition(part.name, version).addRole(SequenceOntology.GENE);
        break;
      case PROMOTER:
        doc.getComponentDefinition(part.name, version).addRole(SequenceOntology.PROMOTER);
        break;
      case TERMINATOR:
        doc.getComponentDefinition(part.name, version).addRole(SequenceOntology.TERMINATOR);
        break;
      default:
        break;
    }
